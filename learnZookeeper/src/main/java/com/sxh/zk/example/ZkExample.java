...
/**
 * Zk Api Demo
 * 删除、修改、查询所用的version版本号，分布式事务锁的实现方式，乐观锁。ACL权限：所有人、限定人、限定IP等
 * @author 七脉
 * @link https://www.cnblogs.com/zwcry/p/10407806.html
 * @date 2021/7/30
 */
public class ZkExample{

    private static Logger log = LoggerFactory.getLogger(ZkExample.class);

    //集群节点
    public static final String zkServerClusterConnect = "localhost:2181,localhost:2182,localhost:2183";

    //单一节点
    public static final String zkServerSingleConnect = "localhost:2181";
...
//        ZooKeeper zk2 = reconnect(sessionId, sessionPasswd);
    ...
    public static ZooKeeper connect() throws IOException, InterruptedException{
        ...
        log.info("准备建立zk服务");
        ZooKeeper zk = new ZooKeeper(zkServerClusterConnect, timeout, new ZkWatcher(cdl,"建立连接"));
        log.info("完成建立zk服务");
    ...
    public static ZooKeeper reconnect(long sessionId, byte[] sessionPasswd) throws IOException, InterruptedException{
        CountDownLatch cdl = new CountDownLatch(1);
        log.info("准备重新连接zk服务");
        ZooKeeper zk = new ZooKeeper(zkServerClusterConnect, timeout, new ZkWatcher(cdl,"重新连接"), sessionId, sessionPasswd);
        log.info("完成重新连接zk服务");
        cdl.await();//这里为了等待wather监听事件结束
        return zk;
    }
